...
function getSyncStatusText(syncing: boolean, syncEnabled: boolean, t: TranslateFn): string {
  if (syncing) {
    return t('syncing');
  }
  return syncEnabled ? t('synced') : t('disconnected');
}
      ...
      <div className="flex items-center gap-1.5 xs:gap-2 sm:gap-3 md:gap-4 overflow-hidden">
        {userId ? (
          <div className="flex items-center gap-1 xs:gap-1.5 sm:gap-2 min-w-0">
            <div className="flex items-center gap-0.5 xs:gap-1 min-w-0">
              ...
              <span className="hidden sm:inline truncate">
                {getSyncStatusText(!!syncing, !!syncEnabled, t)}
              </span>
          ...
          <button
            ...
            title={t('signInToSyncStatus')}
            aria-label={t('signInToSyncStatus')}
          >
            <HardDrive className="h-2.5 w-2.5 xs:h-3 xs:w-3 flex-shrink-0" />
            <span className="hidden md:inline truncate">{t('localModeClickToSync')}</span>
            <span className="md:hidden truncate">{t('local')}</span>
          </button>
        )}
        ...
        {activeNoteId && (
          <div className="hidden lg:flex items-center gap-0.5 xs:gap-1">
            ...
            <span className="truncate">{t('lastModified', { time: new Date().toLocaleTimeString(locale) })}</span>
          </div>
        )}
      </div>
