...
function getNoteBackgroundClass(
...
function TrashFolderItem({
  ...
  readonly notesInFolder: readonly Note[];
...
function NoteMultiSelectHeader({
  folderId,
  noteMultiSelectMode,
  toggleNoteMultiSelectMode,
  isAllSelected,
  isSomeSelected,
  onSelectAll,
  onDeselectAll,
              ...
              {trashFolders.map(folder => {
                ...
                return (
                  <TrashFolderItem
                    ...
                    onRestore={e => {
                      handleRestoreFolder(folder.id, e);
                    }}
                    onPermanentlyDelete={e => {
                      handlePermanentlyDeleteFolder(folder.id, e);
                    }}
                  />
                );
              })}
                ...
                .map(note => {
                  ...
                  return (
                    <TrashNoteItem
                      ...
                      onPermanentlyDelete={e => {
                        handlePermanentlyDeleteNote(note.id, e);
                      }}
                    />
                  );
                })}
...
function LeftSidebarContent(props: LeftSidebarContentProps) {
  const {
    ...
    noteMultiSelectMode,
    toggleNoteMultiSelectMode,
    isAllNotesSelectedInFolder,
    isSomeNotesSelectedInFolder,
    selectAllNotesInFolder,
    ...
    <div className="w-full h-full bg-neutral-100 dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-1.5 xs:p-2 sm:p-2.5 md:p-3 laptop-sm:p-3 laptop:p-4">
            ...
            {folderMultiSelectMode && (
              ...
              >
                <ThreeStateCheckbox
                  ...
                  someSelected={isSomeFoldersSelected()}
                />
              </button>
            )}
                ...
                {isExpanded && (
                  <div className="ml-3 xs:ml-4 space-y-0.5 xs:space-y-1 mt-0.5 xs:mt-1">
                    ...
                    {folderNotes.length > 0 && (
                      <NoteMultiSelectHeader
                        folderId={folder.id}
                        noteMultiSelectMode={noteMultiSelectMode}
                        toggleNoteMultiSelectMode={toggleNoteMultiSelectMode}
                        isAllSelected={isAllNotesSelectedInFolder(folder.id)}
                        isSomeSelected={isSomeNotesSelectedInFolder(folder.id)}
                        onSelectAll={() => {
                          selectAllNotesInFolder(folder.id);
                        }}
                        onDeselectAll={deselectAllNotesInFolder}
                        selectedCount={selectedNoteIds.size}
                        hasSelectedInView={folderNotes.some(n => selectedNoteIds.has(n.id))}
          ...
          <div className="ml-3 xs:ml-4 space-y-0.5 xs:space-y-1">
            ...
            {notes.some(n => !n.folder_id) && (
              <NoteMultiSelectHeader
                ...
                noteMultiSelectMode={noteMultiSelectMode}
                toggleNoteMultiSelectMode={toggleNoteMultiSelectMode}
                isAllSelected={isAllNotesSelectedInFolder('root')}
                isSomeSelected={isSomeNotesSelectedInFolder('root')}
                onSelectAll={() => {
                  selectAllNotesInFolder('root');
                }}
                onDeselectAll={deselectAllNotesInFolder}
                selectedCount={selectedNoteIds.size}
                hasSelectedInView={notes
                  .filter(n => !n.folder_id)
                  .some(n => selectedNoteIds.has(n.id))}
                onDeleteSelected={handleDeleteSelectedNotes}
              />
            )}
            {notes
              .filter(n => !n.folder_id)
              .map(note => renderNote(note, true, noteMultiSelectMode))}
          </div>
        )}
        ...
        <TrashSection
          ...
          deselectAllTrashItems={deselectAllTrashItems}
          selectAllTrashItems={selectAllTrashItems}
          selectedTrashItems={selectedTrashItems}
          handleRestoreSelected={handleRestoreSelected}
          handleDeleteSelected={handleDeleteSelected}
          handleEmptyTrash={handleEmptyTrash}
          toggleTrashItemSelection={toggleTrashItemSelection}
          handleRestoreFolder={handleRestoreFolder}
          handlePermanentlyDeleteFolder={handlePermanentlyDeleteFolder}
...
export default function Sidebar({
  ...
  const permanentlyDeleteTrashItem = async (itemId: string) => {
    ...
    if (
      !/^folder-[a-zA-Z0-9-_]{1,100}$/.test(itemId) &&
      !/^note-[a-zA-Z0-9-_]{1,100}$/.test(itemId)
    ) {
      console.error('Invalid itemId format:', itemId);
      return;
    }
    const id = isFolder ? itemId.replace('folder-', '') : itemId.replace('note-', '');
    if (isFolder && permanentlyDeleteFolder) {
      return permanentlyDeleteFolder(id);
    }
    if (!isFolder && permanentlyDeleteNote) {
  ...
  const restoreTrashItem = async (itemId: string) => {
    ...
    const id = isFolder ? itemId.replace('folder-', '') : itemId.replace('note-', '');
    if (isFolder && restoreFolder) {
      return restoreFolder(id);
    }
    if (!isFolder && restoreNote) {
      ...
      {editingNoteId === note.id ? (
        ...
        <div className="flex items-center w-full min-h-[44px] gap-1">
          ...
          >
            ...
            <span
              className={`text-xs xs:text-sm sm:text-sm laptop:text-sm truncate ${
  ...
  if (side === 'left') {
    return (
      <LeftSidebarContent
        ...
        folders={folders}
        notes={notes}
        folderMultiSelectMode={folderMultiSelectMode}
        toggleFolderMultiSelectMode={toggleFolderMultiSelectMode}
        isAllFoldersSelected={isAllFoldersSelected}
        isSomeFoldersSelected={isSomeFoldersSelected}
        ...
        noteMultiSelectMode={noteMultiSelectMode}
        toggleNoteMultiSelectMode={toggleNoteMultiSelectMode}
        isAllNotesSelectedInFolder={isAllNotesSelectedInFolder}
        isSomeNotesSelectedInFolder={isSomeNotesSelectedInFolder}
        selectAllNotesInFolder={selectAllNotesInFolder}
        deselectAllNotesInFolder={deselectAllNotesInFolder}
