...
const MarkdownH1 = ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
  <h1 className="text-4xl font-bold mb-4 mt-8 text-gray-900 dark:text-gray-100" {...props}>
    {children}
  </h1>
);

const MarkdownH2 = ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
  <h2 className="text-3xl font-semibold mb-3 mt-6 text-gray-900 dark:text-gray-100" {...props}>
    {children}
  </h2>
);

const MarkdownH3 = ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
  <h3 className="text-2xl font-semibold mb-2 mt-4 text-gray-900 dark:text-gray-100" {...props}>
    {children}
  </h3>
);
...
export default function MarkdownPreview({ content, onWikiLinkClick }: MarkdownPreviewProps) {
  ...
  useEffect(() => {
    if (!previewRef.current || !onWikiLinkClick) {
      return;
    }
