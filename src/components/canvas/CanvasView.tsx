...
function getNodeBorderClass(isSelected: boolean, isInMultiSelection: boolean): string {
  ...
  if (isInMultiSelection) {
    return 'border-primary-400 shadow-xl ring-2 ring-primary-300 ring-opacity-50';
  }
...
export default function CanvasView({
  ...
  const isValidNode = useCallback(
    (node: CanvasNode) => node.type !== 'note' || notes.some(n => n.id === node.id),
    [notes]
  );

  // Helper: create initial canvas nodes from notes
  const createInitialNodes = useCallback(
    (notesList: readonly Note[]): CanvasNode[] =>
      notesList
  ...
  const processCanvasData = useCallback(
    async (savedNodes: CanvasNode[] | null) => {
  ...
  const syncExistingNodes = useCallback(
    (nextNodes: CanvasNode[], currentNoteIds: Set<string>): boolean => {
      ...
      for (let i = nextNodes.length - 1; i >= 0; i--) {
        ...
        if (inNotes) {
          ...
          const contentChanged =
            sourceNote && (node.content !== sourceNote.content || node.title !== sourceNote.title);
          if (contentChanged) {
            nextNodes[i] = {
              ...node,
  ...
  const addMissingNotes = useCallback(
    (nextNodes: CanvasNode[], existingNodeIds: Set<string>): boolean => {
  ...
  const updateNodeContent = (nodeId: string, newContent: string) => {
    setCanvasNodes(prev => prev.map(n => (n.id === nodeId ? { ...n, content: newContent } : n)));
  };
